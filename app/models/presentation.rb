...
class Presentation < ActiveRecord::Base
  ...
  def self.generate_scorm_manifest(version,pjson,presentation,options={})
    ...
    if presentation and !presentation.id.nil?
      ...
      lomIdentifier = "urn:MOVLE:" + identifier
    else    
    ...
    when "12"
      #SCORM 1.2
      manifestHeaderOptions = {
        "identifier"=>"MOVLE_PRESENTATION_" + identifier,
        "version"=>"1.0",
  ...
  def self.generate_LOM_metadata(pjson, presentation, options={})
    ...
    supportedLOMSchemas = ["custom","loose","MOVLE"]
    ...
    myxml.tag!("lom",lomHeaderOptions) do
      ...
      myxml.general do
        ...
        myxml.description do
          if pjson["description"]
            ...
            myxml.string(pjson["title"] + ". Web Presentation provided by " + Movle::Application.config.full_domain + ".", :language=> metadataLanguage)
          else
            myxml.string("Web Presentation provided by " + Movle::Application.config.full_domain + ".", :language=> metadataLanguage)
          end
        end
      ...
      myxml.rights do
        ...
        myxml.description do
          ...
          myxml.string(license + "For additional information or questions regarding copyright, distribution and reproduction, visit " + Movle::Application.config.full_domain + "/terms_of_use .", :language=> metadataLanguage)
        end
      end
  ...
  def parse_for_metadata
    ...
    self.thumbnail_url = (parsed_json["avatar"] ? parsed_json["avatar"] : Movle::Application.config.full_domain + "/assets/logos/original/presentation-00.png")
  end
