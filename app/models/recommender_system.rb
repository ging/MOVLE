...
class RecommenderSystem
  ...
  def self.getPreselection(options={})
    ...
    # Repeated resources.
    options[:preselection][:ids_to_avoid] = [options[:lo_profile][:id_repository]] if options[:lo_profile][:id_repository]


    # Search/query options
    searchOptions = {}
    searchOptions[:n] = [options[:settings][:database][:preselection_size],MOVLE::Application.config.rs_settings[:database][:max_preselection_size]].min
    ...
    searchOptions[:ids_to_avoid] = options[:preselection][:ids_to_avoid] || [-1]

    # Get preselection from database
    searchOptions[:models].each do |model|
      preSelection += model.limit(searchOptions[:n]).public_items.where("id not in (?)",searchOptions[:ids_to_avoid]).order(MOVLE::Application.config.agnostic_random)
    end
