...
class RecommenderSystem
  ...
  def self.prepareOptions(options={})
    options = {:n => 10, :user_profile => {}, :lo_profile => {}, :settings => Movle::Application.config.rs_settings.recursive_merge({})}.recursive_merge(options)
    options[:user_profile][:los] = options[:user_profile][:los].first(options[:max_user_pastlos] || Movle::Application.config.max_user_pastlos).sample(options[:max_user_los] || Movle::Application.config.max_user_los) unless options[:user_profile][:los].blank?
    options
  end
  ...
  def self.getPreselection(options={})
    ...
    # Repeated resources.
    options[:preselection][:movle_ids_to_avoid] = [options[:lo_profile][:id_repository]] if options[:lo_profile][:id_repository]


    # Search/query options
    searchOptions = {}
    searchOptions[:n] = [options[:settings][:movle_database][:preselection_size],Movle::Application.config.rs_settings[:movle_database][:max_preselection_size]].min
    ...
    searchOptions[:movle_ids_to_avoid] = options[:preselection][:movle_ids_to_avoid] || [-1]

    # Get preselection from database
    searchOptions[:models].each do |model|
      preSelection += model.limit(searchOptions[:n]).public_items.where("id not in (?)",searchOptions[:movle_ids_to_avoid]).order(Movle::Application.config.agnostic_random)
    end
  ...
  def self.getSemanticDistance(textA,textB)
    ...
    textA = textA.first(Movle::Application.config.max_text_length)
    textB = textB.first(Movle::Application.config.max_text_length)
  ...
  def self.IDF(word)
    # Math::log((2+Movle::Application.config.repository_total_entries)/(1+(Movle::Application.config.words[word] || 0)).to_f)
  ...
  def self.getRSUserSetting(settingName,settingFamily,options={})
    ...
    if userSettings.blank?
      ...
      case settingFamily
      when "weights"
        movleRSConfig = Movle::Application.config.weights
      when "filters"
        movleRSConfig = Movle::Application.config.filters
      end
      userSettings = movleRSConfig[defaultKey] unless movleRSConfig.nil?
    end
  ...
  # The current default weights can be accesed in the Movle::Application.config.weights variable.
  ...
  # The current default filters can be accesed in the Movle::Application.config.filters variable.
