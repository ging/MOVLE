class Scormfile < ActiveRecord::Base
  ...
  def url_full
    Movle::Application.config.full_code_domain + "/code/scormfiles/" + self.id.to_s + "/scorm_wrapper.html"
  end
  ...
  def extract_los
    ...
    Scorm::Package.open(zipFile, :cleanup => true, :force_cleanup_on_close => true) do |pkg|
      pkg.manifest.resources.each_with_index do |resource,i|
        ...
        lo.hreffull = Movle::Application.config.full_code_domain + "/code/scormfiles/" + self.id.to_s + "/" + lo.href
  ...
  def unpackage
    ...
    #Create folders
    if Movle::Application.config.APP_CONFIG["code_path"].nil?
      scormPackagesDirectoryPath = Rails.root.join('public', 'code', 'scormfiles').to_s
    else
      scormPackagesDirectoryPath = Movle::Application.config.APP_CONFIG["code_path"] + "/scormfiles"
    end
    ...
    self.update_column(:lohreffull, Movle::Application.config.full_code_domain + "/code/scormfiles/" + self.id.to_s + "/" + self.lohref)
    ...
    loDirectoryPathToSave.slice! (Rails.root.to_s+"/") if Movle::Application.config.APP_CONFIG["code_path"].nil?
