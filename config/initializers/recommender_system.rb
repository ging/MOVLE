...
Rails.application.configure do
  ...
  #Default settings to use in RS
  config.rs_settings = {}
  config.rs_settings = rsConfig[:default_settings] unless rsConfig[:default_settings].blank?
  config.rs_settings = {:database => {:max_preselection_size => 500, :preselection_size => 500}, :max_text_length => 50, :max_user_los => 1, :max_user_pastlos => 4, :preselection_filter_resource_type => "true", :preselection_filter_languages => "false"}.recursive_merge(config.rs_settings)
